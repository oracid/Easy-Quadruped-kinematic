/* This program tests a stroll with Kinematic() function - by Oracid. 03/01/2019 */
#include <Servo.h>

Servo mySrv[8];                 // Indicate how many servos. Here 8 servos.
const int nbFeet = 11;          // Indicate here the number of values of each line of the table bellow. Here 11.
...
int SrvAdjust[nbSrv]={ 0, 5, 0, 9, 0, 2, -2, 8 };   // each servo could need an adjustement for neutral position(90°)                        
int LeftRight=1;  
...
//  Serial.print("nbSrv = ");Serial.print(nbSrv);Serial.print("  lgTabl = ");Serial.print(lgTabl);Serial.print("  Foot = ");Serial.print(Foot);Serial.print("  Speed = ");Serial.println(Speed);
  ...
  mySrv[6].attach(6);
  mySrv[7].attach(7);
//  Kinematic(&lgTab, Forward[0], &nbSrv, &Speed, &Foot, &SrvAdjust[0] );   // uncomment out to test only one cycle
}
...
void  loop() {
  ...
  LeftRight=LeftRight*(-1);         //once turn right, then turn left
  if( LeftRight > 0 ) {
    for(int i=0;i<3;i++)  Kinematic(&lgTab, TurnRight[0], &nbSrv, &Speed, &Foot, &SrvAdjust[0] ); 
  }else{
    for(int i=0;i<3;i++)  Kinematic(&lgTab, TurnLeft[0], &nbSrv, &Speed, &Foot, &SrvAdjust[0] );    
  }
}
